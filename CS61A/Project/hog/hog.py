...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    score, i, has_one = 0, 0, False
    while i < num_rolls:
        side = dice()
        if side == 1:
            has_one = True
        score, i = score + side, i + 1
    if has_one:
        score = 1
    return score
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    points = 3 * abs(player_ones_digit - opponent_tens_digit)
    if points == 0:
        points = 1
    return points
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # is not Sus
    if factor_count != 3 and factor_count != 4:
        return score
    # is Sus
    greater_closest_prime = score + 1
    while not is_prime(greater_closest_prime):
        greater_closest_prime += 1
    return greater_closest_prime
    # END PROBLEM 4
